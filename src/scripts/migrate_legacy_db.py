...
if __name__ == "__main__":
    ...
    for index, row in legacy_df.iterrows():
        ...
        old_formatted_extracted_date = datetime.strptime(
            row["extracted_date"], "%B %d, %Y %I:%M %p"
        )
        old_formatted_published_date = datetime.strptime(
            row["published_date"], "%B %d, %Y %I:%M %p"
        )
        ...
        loop_tt_news_article = looptt_articles_pb2.LoopTTNewsArticle(
            id=row["id"],
            type="news_article",
            created_date=old_formatted_published_date.replace(
                tzinfo=timezone.utc
            ).timestamp(),
            fields=looptt_articles_pb2.LoopTTNewsArticle.NewsArticleFields(
                title=row["title"],
                url=f"{row['source']}{row['url']}",
                extracted_date=old_formatted_extracted_date.replace(
                    tzinfo=timezone.utc
                ).timestamp(),
                published_date=old_formatted_published_date.replace(
                    tzinfo=timezone.utc
                ).timestamp(),
        ...
        loop_tt_text_content = looptt_articles_pb2.ArticlesTextContent(
            ...
            fields={"title": row["title"], "text": row["content"]},
        )

        loop_tt_news_articles.append(
            {"article": loop_tt_news_article, "text_content": loop_tt_text_content}
        )

    logger.info(f"Extracted {len(loop_tt_news_articles)} from csv")

    articles = looptt_articles_pb2.LoopTTArticles(
        articles=[article["article"] for article in loop_tt_news_articles]
    )
    ...
    logger.info(
        f"Found {len(unique_articles_to_insert)} unique articles that are not in the postgres db"
    )

    unique_articles_messages_to_insert: list[Message] = [
        content["article"] for content in unique_articles_to_insert
    ]
    unique_text_content_messages_to_insert: list[Message] = [
        content["text_content"] for content in unique_articles_to_insert
    ]

    logger.info(
        f"Trying to insert {len(unique_articles_messages_to_insert)} loop tt articles into the database"
    )
    inserted_articles = insert_multiple_looptt_articles_posts_db(
        unique_articles_messages_to_insert, secrets
    )
    logger.info(f"Inserted {inserted_articles} articles into the database")

    logger.info(
        f"Trying to insert {len(unique_text_content_messages_to_insert)} text content into the database"
    )
    inserted_text_content = insert_multiple_article_text_content_db(
        unique_text_content_messages_to_insert, secrets
    )
